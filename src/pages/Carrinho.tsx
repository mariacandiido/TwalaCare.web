...
export function Carrinho() {
  ...
  useEffect(() => {
    ...
    setQuantityByKey((prev) => ({ ...prev, ...next }));
  }, [items]);

  const subtotal = items.reduce((sum, item) => {
    const q = quantityByKey[item.id] ?? item.quantity;
    return sum + item.price * q;
  }, 0);
  const total = subtotal + DELIVERY_FEE;

  const handleQuantityChange = (id: string, delta: number) => {
    const current = quantityByKey[id] ?? items.find((i) => i.id === id)?.quantity ?? 1;
    const newQuantity = current + delta;
    if (newQuantity < 1) return;
    setQuantityByKey((prev) => ({ ...prev, [id]: newQuantity }));
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center justify-between mt-4">
                    <div className="flex items-center gap-3">
                      ...
                      >
                        {quantityByKey[item.id] ?? item.quantity}
                      </span>
                      <button
                        type="button"
                        onClick={(e) => {
